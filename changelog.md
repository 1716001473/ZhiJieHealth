...
## 2026-02-14 敏感词检测逻辑修复与体验优化 🛡️🔧

### 功能概述

修复了微信小程序敏感词检测的逻辑缺陷，并优化了前端错误提示体验。现在修改个人信息时，若包含敏感词会准确提示具体错误原因，而非笼统的“保存失败”。

### 核心改进

**1. 后端敏感词检测修复**
- **逻辑顺序调整**：将本地敏感词拦截逻辑（如“测试”、“色情”）移至微信 API 调用之前。
  - **修复前**：若未配置 `WECHAT_APP_ID`，函数直接返回 True，导致本地敏感词拦截失效。
  - **修复后**：优先执行本地拦截，无论是否配置微信 AppID，敏感词均会被拦截。
- **500 错误修复**：修复了在重构过程中意外删除 `msg_sec_check` 函数定义导致的 `NameError`。

**2. 前端错误提示优化**
- **精准报错**：在 `edit.vue` 中增加对后端 `400 Bad Request` 响应的处理。
  - **优化前**：接口报错时统一提示“保存失败”，用户无法感知具体原因。
  - **优化后**：优先提取后端返回的 `detail` 字段（如“昵称包含违规内容，请修改”），直接展示给用户。

---

## 2026-02-14 UI 细节优化与通过 AI 食谱修复 🎨🐛

### 功能概述

进一步打磨应用 UI 细节，统一了登录页视觉风格，优化了饮食记录页的交互体验，并修复了 AI 食谱详情页的显示问题。

### 核心改进

**1. 登录页 UI 升级**
- **风格统一**：头部背景改为与 App 主题一致的绿色渐变（`#4CAF50` → `#81C784`）。
- **视觉优化**：标题文字颜色改为白色，去除容器内边距以实现头部全宽显示，整体更加清爽大气。
- **样式隔离**：为登录页和个人中心页添加 `scoped`样式，彻底解决页面切换导致的样式污染（Style Pollution）问题。

**2. 饮食记录页交互增强**
- **日期选择优化**：将原本简单的"切换日期"文本升级为原生日期选择器。
- **回到今天**：新增智能"回到今天"按钮，仅在非今日日期显示，支持一键快速返回，提升浏览历史记录时的便捷性。

**3. AI 食谱详情页修复**
- **封面图修复**：修复了 AI 生成食谱封面图路径为 404 的问题，增加了智能回退逻辑，自动替换失效的本地路径为高清在线美食图。
- **体验优化**：即使后端返回了不存在的 `/static/ai_plan.png`，前端也能优雅降级显示精美配图。

---

## 2026-02-13 数据库迁移 MySQL + 接口限流与安全升级 🛡️🚀

### 功能概述

完成生产环境架构升级，将数据库从 SQLite 迁移至 MySQL，并全面增强系统安全性，包括接口防暴力破解限流和密钥安全校验。

### 核心改进

**1. 数据库迁移 (SQLite → MySQL)**
- **全量迁移**：完成 14 张表、596 条数据的无损迁移，包括用户数据、食物库、饮食记录等。
- **架构升级**：生产环境正式切换至 MySQL 8.0，支持更高并发和数据可靠性。
- **连接池优化**：配置 SQLAlchemy 连接池 (`pool_size=5`, `pool_recycle=3600`)，解决高并发下的连接复用问题。

**2. 接口限流 (Rate Limiting)**
- **方案**：集成 `slowapi` 中间件，基于 IP 进行流量控制，防止暴力破解和恶意刷接口。
- **策略配置**：
  - 🛑 **登录接口** (`/login`)：**5 次/分钟** (防撞库)
  - 🛑 **注册接口** (`/register`)：**3 次/分钟** (防批量注册)
  - 🛑 **修改密码** (`/password`)：**5 次/分钟**
  - 🌍 **全局默认**：60 次/分钟

**3. 安全加固 (Security Hardening)**
- **密钥校验**：应用启动时自动检查 `SECRET_KEY` 安全性。
  - **开发环境**：若未配置，自动生成随机强密钥并警告。
  - **生产环境**：若未配置或密钥过短，**拒绝启动**，强制要求配置安全密钥。
- **配置隔离**：`.env` 明确区分开发/生产数据库配置，移除硬编码默认值。

**4. 基础设施升级**
- **新增依赖**：`pymysql` (MySQL 驱动), `cryptography`, `slowapi`。
- **移除依赖**：`aiosqlite` (不再需要)。
- **清理**：归档旧 SQLite 数据库 (`.db.bak`)，清理临时迁移脚本。

---

## 2026-02-10 统一饮食录入体验 + 智能单位推断 🍱⚖️✨

### 功能概述

全面统一了 App 内所有饮食录入入口（搜索、拍照、食谱）的交互体验，引入"智能单位"模式，让用户可以用自然语言习惯（如"1 碗"、"1 个"）快速记录饮食，同时升级了拍照识别页的视觉设计。

### 核心改进

**1. 统一饮食录入 UI (Unified Food Entry)**
- **问题**：在此之前，搜索添加、拍照识别结果、精选食谱详情页的"添加饮食"弹窗样式和逻辑各不相同，体验割裂。
- **方案**：重构了所有相关弹窗，统一使用 **"智能单位 + 克数切换"** 的新标准组件。
  - **搜索添加页** (`record/add.vue`)：✅ 已统一
  - **拍照识别页** (`result/index.vue`)：✅ 已统一
  - **精选食谱页** (`recipe/detail.vue`)：✅ 已统一

**2. 智能单位推断 (Smart Unit Inference)**
- **功能**：系统现在能根据食物名称自动推断最合适的计量单位，通过 `FoodService._infer_unit` 实现。
- **效果**：
  - 🍚 **米饭/粥/面** → 默认单位 **"碗"** (约 200-300g)
  - 🍎 **苹果/鸡蛋/饺子** → 默认单位 **"个"** (智能匹配重量)
  - 🥛 **牛奶/豆浆/咖啡** → 默认单位 **"杯"** (约 250g)
  - 🥩 **牛排/鸡胸** → 默认单位 **"块"**
- **双模式切换**：用户可以在 **"智能单位"** (如 1.5 碗) 和 **"克数输入"** (如 300g) 之间一键切换。

**3. 拍照识别页视觉升级**
- **旧版**：图片由绿色导航栏下方开始，下方内容区有边距，存在割裂感。
- **新版**：采用 **"沉浸式全宽图片 + 白色圆角卡片上叠"** 的现代化布局。
  - 图片高度增加至 `450rpx`，全宽显示。
  - 内容区域使用 `-40rpx` 的负边距向上层叠，配合大圆角设计，层次感更强，视觉体验对标主流美食 App。

**4. 后端数据增强**
- **Schema 升级**：`FoodResponse` 增加 `default_unit` 和 `unit_weight` 字段。
- **服务升级**：`FoodService` 新增 `_infer_unit` 方法，支持基于关键词和分类的单位推断规则。
